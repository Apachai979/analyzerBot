...
class TrendFilter12hConfig:
    ...
    # Минимальный допустимый рост EMA200 за lookback
    # Пример: 0.001 = +0.1%
    min_ema200_slope_pct: float = 0.001

    # Параметры свингов
    swing_left_bars: int = 2
    swing_right_bars: int = 2

    # Сколько soft-условий должно пройти
...
# =========================
# Вспомогательная структура swing
# =========================

@dataclass(frozen=True, slots=True)
class SwingPoint:
    # Позиция в DataFrame по номеру строки
    pos: int
    # Исходный df.index этой свечи
    index: Any
    # Цена swing high / low
    price: float
...
def find_confirmed_swings(
    ...
    Возвращает:
    - highs: список SwingPoint
    - lows: список SwingPoint
    """
    ...
    for i in range(left_bars, n - right_bars):
        ...
        if h[i] == np.max(high_window) and np.sum(high_window == h[i]) == 1:
            highs.append(SwingPoint(pos=i, index=idx[i], price=float(h[i])))

        if l[i] == np.min(low_window) and np.sum(low_window == l[i]) == 1:
            lows.append(SwingPoint(pos=i, index=idx[i], price=float(l[i])))

    return highs, lows


def serialize_swing_points(points: list[SwingPoint] | None) -> list[dict[str, Any]] | None:
    """
    Преобразует SwingPoint в сериализуемый формат для логов.
    """
    if points is None:
        return None
...
def evaluate_market_structure(
    ...
    """
    Более строгая, но все еще практичная оценка структуры тренда.

    Логика:
    - higher_lows сами по себе уже хороший признак силы
    - higher_highs считаем сильным признаком только если цена
      уже выше последнего подтвержденного swing high

    Итого:
    structure_ok = higher_lows OR (higher_highs AND close_above_last_swing_high)
    """
    last_two_highs = swing_highs[-2:] if len(swing_highs) >= 2 else None
    last_two_lows = swing_lows[-2:] if len(swing_lows) >= 2 else None

    higher_highs = False
    higher_lows = False
    close_above_last_swing_high = False

    last_swing_high_age_bars = None
    last_swing_low_age_bars = None

    if last_two_highs is not None:
        higher_highs = last_two_highs[-1].price > last_two_highs[-2].price

    if last_two_lows is not None:
        higher_lows = last_two_lows[-1].price > last_two_lows[-2].price

    if len(swing_highs) >= 1:
        last_confirmed_high = swing_highs[-1]
        close_above_last_swing_high = last_close > last_confirmed_high.price
        last_swing_high_age_bars = last_pos - last_confirmed_high.pos

    if len(swing_lows) >= 1:
        last_confirmed_low = swing_lows[-1]
        last_swing_low_age_bars = last_pos - last_confirmed_low.pos

    structure_ok = bool(
        higher_lows or
        (higher_highs and close_above_last_swing_high)
    )

    return {
        "enough_highs": len(swing_highs) >= 2,
        "enough_lows": len(swing_lows) >= 2,
        "last_two_swing_highs": serialize_swing_points(last_two_highs),
        "last_two_swing_lows": serialize_swing_points(last_two_lows),
        "higher_highs": higher_highs,
        "higher_lows": higher_lows,
        "close_above_last_swing_high": close_above_last_swing_high,
        "last_swing_high_age_bars": last_swing_high_age_bars,
        "last_swing_low_age_bars": last_swing_low_age_bars,
...
def trend_filter_12h(
    ...
    2. Проверяем дополнительные подтверждения (soft conditions)
    3. Если hard-условия выполнены и soft_score >= порога,
       разрешаем искать сетап на 4h / триггер на 1h
    ...
    # Минимум истории нужен, чтобы EMA200, ATR, ADX и slope были адекватными
    required_rows = max(
    ...
    # 3) EMA200 имеет не просто положительный, а минимально значимый наклон
    ema_slow_past = data["ema_slow"].iloc[-1 - config.ema_slope_lookback]

    if pd.isna(ema_slow_past) or ema_slow_past <= 0:
        ema200_slope_pct = None
        ema200_slope_up = False
    else:
        ema200_slope_pct = float((last["ema_slow"] / ema_slow_past) - 1.0)
        ema200_slope_up = bool(ema200_slope_pct >= config.min_ema200_slope_pct)
    ...
    # 3) Цена выше EMA50
    # Это полезно: не пускать глубокую коррекцию как "здоровый тренд"
    close_above_ema50 = bool(last["close"] > last["ema_mid"])

    # Доп. диагностическое поле: направление DI
    plus_di_bullish = bool(
    ...
    details: dict[str, Any] = {
        ...
        "last_candle": {
            ...
            "plus_di_bullish": plus_di_bullish,
        },
        ...
        "ema200_slope": {
            ...
            "ema200_past": float(ema_slow_past) if pd.notna(ema_slow_past) else None,
            "slope_pct": ema200_slope_pct,
            "min_required_slope_pct": config.min_ema200_slope_pct,
