...
def safe_round(value: Any, digits: int = 4) -> Any:
    """Округляет число для отчета, не ломаясь на None/NaN."""
    if value is None:
        return None
...
def fetch_spot_usdt_symbols() -> list[str]:
    ...
    response = bybit_client._request("get_instruments_info", category="spot")
    if not bybit_client._response_ok(response, "getting spot instruments"):
        return []
...
def get_coinmarketcap_data_batched(symbols: list[str], batch_size: int = 80) -> dict[str, dict[str, Any]]:
...
def refresh_filtered_symbols(
    ...
    eligible_coins.sort(
        key=lambda item: (item["market_cap"], item["volume_24h"]),
        reverse=True,
    )
...
def resolve_dynamic_rank(
    ...
    """Возвращает приоритет символа для dynamic universe или None, если символ не должен попасть в список."""
    validate_dynamic_stage(min_stage)
...
def write_report_tsv(report_rows: list[dict[str, Any]], output_file: str) -> None:
    """Сохраняет calibration-report в TSV-формате, удобном для batch-анализа и пересборки dynamic symbols."""
    output_path = Path(output_file)
    output_path.parent.mkdir(parents=True, exist_ok=True)
...
def build_dynamic_symbols_from_report_file(report_file: str, min_stage: str = "setup") -> list[str]:
    ...
    with report_path.open("r", encoding="utf-8", newline="") as handle:
        ...
        for row in reader:
            ...
            rank = resolve_dynamic_rank(
                trend_passed=row.get("trend_passed") == "True",
                setup_passed=row.get("setup_passed") == "True",
...
def analyze_symbol(
    ...
    """Прогоняет один символ через 12H -> 4H -> 1H и возвращает компактный calibration-report."""
    ...
    return {
        ...
        "4H": {
            ...
            "current_extension_atr": safe_round(setup_result.details.get("pullback", {}).get("current_extension_atr"), 3),
        },
        "1H": {
...
def print_summary_table(report_rows: list[dict[str, Any]]) -> None:
    """Печатает компактную таблицу по всем символам."""
    headers = [
        "SYMBOL",
        "12H",
        "4H",
...
def parse_args() -> argparse.Namespace:
    """Парсит CLI-параметры calibration report script."""
    parser = argparse.ArgumentParser(
        description="Run a compact 12H -> 4H -> 1H calibration report for a list of symbols.",
    )
    parser.add_argument(
        "symbols",
        nargs="*",
        help="Symbols to analyze, for example: TRXUSDT VVVUSDT CFGUSDT",
    )
    parser.add_argument(
        "--symbols-file",
    ...
    parser.add_argument(
        "--refresh-filtered-symbols",
        action="store_true",
        help="Refresh data/filtered_symbols.txt from current Bybit spot USDT instruments and CMC filters.",
    )
    parser.add_argument(
        "--update-dynamic-symbols",
        action="store_true",
        help="Write data/dynamic_symbols.txt from calibration results.",
    )
    parser.add_argument(
        "--dynamic-min-stage",
        choices=["trend", "setup", "entry"],
        default="setup",
        help="Minimum stage required for a token to be kept in dynamic_symbols.txt.",
    )
    parser.add_argument(
        "--filtered-output-file",
        default=FILTERED_SYMBOLS_FILE,
        help="Path for the broad filtered symbols universe.",
    )
    parser.add_argument(
        "--dynamic-output-file",
        default=DYNAMIC_SYMBOLS_FILE,
        help="Path for the актуальный dynamic symbols universe.",
    )
    parser.add_argument("--cmc-min-market-cap", type=float, default=float(MIN_MARKET_CAP))
    parser.add_argument("--cmc-min-volume-24h", type=float, default=float(MIN_CMC_VOLUME_24H))
    parser.add_argument(
        "--include-stablecoins",
        action="store_true",
        help="Keep stablecoins in filtered_symbols universe if they pass market cap and volume filters.",
    )
    parser.add_argument("--limit-12h", type=int, default=400)
    parser.add_argument("--limit-4h", type=int, default=400)
    parser.add_argument("--limit-1h", type=int, default=400)
    parser.add_argument("--trend-soft", type=int, default=2)
    parser.add_argument("--setup-soft", type=int, default=6)
    parser.add_argument("--entry-soft", type=int, default=5)
    parser.add_argument("--entry-max-extension", type=float, default=1.6)
    parser.add_argument(
        "--output-file",
        help="Write a human-readable detailed text report to a file.",
    )
    parser.add_argument(
        "--tsv-output-file",
        default=None,
        help="Write a machine-readable TSV report that can later be reused to rebuild dynamic_symbols.",
    )
    parser.add_argument(
        "--rebuild-dynamic-from-report",
        help="Rebuild dynamic_symbols directly from an existing TSV report without rerunning market analysis.",
    )
    parser.add_argument(
        "--stream-tsv-output-file",
        help="Write TSV rows incrementally during analysis, useful for long batch runs.",
    )
    parser.add_argument(
        "--details",
        action="store_true",
        help="Print per-symbol reasons and failed conditions after the compact summary table.",
    )
    return parser.parse_args()


def main() -> None:
    """Точка входа calibration report script."""
    args = parse_args()

    symbols = list(args.symbols)
    if args.refresh_filtered_symbols:
        refreshed_symbols = refresh_filtered_symbols(
            output_file=args.filtered_output_file,
            min_market_cap=args.cmc_min_market_cap,
            min_volume_24h=args.cmc_min_volume_24h,
            exclude_stablecoins=not args.include_stablecoins,
        )
        symbols.extend(refreshed_symbols)
    if args.symbols_file:
        symbols.extend(load_symbols_from_file(args.symbols_file))
    if (args.update_dynamic_symbols or args.refresh_filtered_symbols) and not args.symbols and not args.symbols_file:
        symbols.extend(load_symbols_from_file(args.filtered_output_file))
    symbols = list(dict.fromkeys(symbols))

    if not symbols and not args.rebuild_dynamic_from_report:
        raise SystemExit("No symbols provided. Pass symbols directly or use --symbols-file.")

    trend_config = TrendFilter12hConfig(
        min_required_rows=260,
        min_soft_conditions_passed=args.trend_soft,
    )
    setup_config = SetupFilter4hConfig(
        min_required_rows=220,
        min_soft_conditions_passed=args.setup_soft,
    )
    entry_config = EntryTrigger1hConfig(
        min_required_rows=180,
        min_soft_conditions_passed=args.entry_soft,
        max_extension_from_ema20_atr=args.entry_max_extension,
    )
    ...
    def build_report_rows() -> list[dict[str, Any]]:
        ...
        stream_handle = None
        stream_writer = None
        if args.stream_tsv_output_file:
            stream_path = Path(args.stream_tsv_output_file)
            stream_path.parent.mkdir(parents=True, exist_ok=True)
            stream_handle = stream_path.open("w", encoding="utf-8", newline="", buffering=1)
            stream_writer = csv.writer(stream_handle, delimiter="\t")
            write_report_tsv_header(stream_writer)
            ...
            for symbol in symbols:
                try:
                    row = analyze_symbol(
                        ...
                        setup_config=setup_config,
                        entry_config=entry_config,
                        limit_12h=args.limit_12h,
                        limit_4h=args.limit_4h,
                        limit_1h=args.limit_1h,
                    )
                except Exception as exc:
                    row = {
                        "symbol": symbol,
    ...
    def emit_report(report_rows: list[dict[str, Any]], include_details: bool = False) -> None:
        print("Calibration config:")
        print(
            f"trend_soft={args.trend_soft} | setup_soft={args.setup_soft} | "
            f"entry_soft={args.entry_soft} | entry_max_extension={args.entry_max_extension}"
        )
        print(f"symbols_count={len(symbols)}")
        if args.refresh_filtered_symbols:
            print(f"filtered_symbols_file={args.filtered_output_file}")
        if args.update_dynamic_symbols:
            print(f"dynamic_symbols_file={args.dynamic_output_file} | min_stage={args.dynamic_min_stage}")
        print()
        print_summary_table(report_rows)
        if include_details:
            print()
    ...
    if args.update_dynamic_symbols:
        if args.rebuild_dynamic_from_report:
            dynamic_symbols = build_dynamic_symbols_from_report_file(
                report_file=args.rebuild_dynamic_from_report,
                min_stage=args.dynamic_min_stage,
            )
        else:
            dynamic_symbols = build_dynamic_symbols(report_rows, min_stage=args.dynamic_min_stage)
        write_symbols_file(args.dynamic_output_file, dynamic_symbols)

    if args.tsv_output_file:
        write_report_tsv(report_rows, args.tsv_output_file)
    ...
    if args.output_file:
        ...
        with output_path.open("w", encoding="utf-8", buffering=1) as file_handle:
            with contextlib.redirect_stdout(file_handle):
                emit_report(report_rows, include_details=True)
        print(f"Report written to {output_path}")
        if args.tsv_output_file:
            print(f"TSV report written to {args.tsv_output_file}")
        if args.stream_tsv_output_file:
            print(f"Streaming TSV report written to {args.stream_tsv_output_file}")
        if args.update_dynamic_symbols:
            print(f"Dynamic symbols written to {args.dynamic_output_file}")
    else:
        if report_rows:
            emit_report(report_rows, include_details=args.details)
        if args.tsv_output_file:
            print(f"TSV report written to {args.tsv_output_file}")
        if args.stream_tsv_output_file:
            print(f"Streaming TSV report written to {args.stream_tsv_output_file}")
