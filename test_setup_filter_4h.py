...
def run(symbol: str = "BTCUSDT", limit_12h: int = 400, limit_4h: int = 400) -> None:
    ...
    if df_12h.empty:
        print(f"После очистки не осталось 12h данных: {symbol}")
        return
    if df_4h.empty:
        print(f"После очистки не осталось 4h данных: {symbol}")
        return
    ...
    print(f"hard_passed={trend_result.hard_passed}")
    print(f"soft_score={trend_result.soft_score}/{trend_result.soft_score_max}")
    print(f"reason={trend_result.reason}")

    print(f"\n=== 4H SETUP FILTER: {symbol} ===")
    print(f"bars={len(df_4h)}")
    print(f"last_closed_candle={df_4h.index[-1]}")
    print(f"passed={setup_result.passed}")
    print(f"hard_passed={setup_result.hard_passed}")
    print(f"setup_state={setup_result.setup_state}")
    print(f"soft_score={setup_result.soft_score}/{setup_result.soft_score_max}")
